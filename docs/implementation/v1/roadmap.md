...
- **Current milestone:** Milestone 11 — User playtest (slices 0–6 complete through `9131095`, plus five 2026-08-18 code-review rounds fully remediated: round one delivered uniform 1–8 client supervision with exact per-endpoint report validation, evidence-derived checkpoint digests/divergence/drop/error/rejection counters with cross-endpoint agreement, and the complete pause-overlay rebinding path; round two ordered terminal observations after shutdown and before closeout finalization with an exit-frame schedule test, implemented client failure categories end-to-end (classification, failure records, panic hook, `configuration` category), decomposed the composed-payload coordinator into its collect/plan/apply/commit stages, replaced the touched modules' broad lint suppressions with item-scoped allows, and made the report reader enforce the full bounded schema; round three moved the launcher's closeout gate onto the binaries' own `validate-closeout` reader instead of an 11-field subset, made idle gamepads unable to claim the active input device, made disabled process diagnostics registration-inert, made `BRAWLER_DIAGNOSTICS_OVERLAY` genuinely force overlay visibility, and item-scoped the combat-subtree and terrain-authority lint exceptions; round four repointed weapon previews and controller lob ranging at the replicated `ResolvedMatchLoadout` (fixing always-hidden previews in network play), made the closeout reader parse every typed field, reconstruct and validate the report, require one shared run identity and profile-correct checkpoint digests, classified all 26 server verification failure paths as `verification-failed` through a shared helper, and moved the settings UI into `client/settings/ui.rs`; round five added the `GameplayAggregatesV1` block to the schema-2 closeout report (match/mode, build, ability, weapon, and terrain summaries consolidated from the process's own telemetry), cached manifest participants during the run so rosters survive shutdown (the committed 2v2 evidence had reported participants=0), derived seed/scripted-action/checkpoint declarations from the selected scenario in the launcher, extended cross-endpoint identity to source revision and the canonical participant roster, removed the retired `ResolvedWeapon` from the client evidence query, and enforced jitter percentile monotonicity; the supervised playtest handoff is delivered, and the user observations, feedback triage, learn-from-errors review, and explicit v1 acceptance remain; see [milestone-11.md](./milestone-11.md))
