...
Last reconciled: **2026-08-24**, after V9 concealment/reveal completed, legacy full-build presets
were identified for removal, and V10 M01 oil-barrel research reached specification review.
...
| CAND-GAME-MODE | One complete additional authoritative mode with compatible map requirements and a player-visible loop | Promoted into active V10; M02 owns one focused `HeistModePlugin` after M01's damageable-target proof. Gem Grab, Solo Showdown, and other modes remain candidates rather than becoming framework requirements | [V10 roadmap](./implementation/v10/roadmap.md); [damageable objects and Heist specification](./18-damageable-world-objects-and-heist.md); [future-version mode candidates](./implementation/v1/roadmap.md#future-version-candidate-backlog); [maps and modes](./04-maps-and-game-modes.md) |
| CAND-ENVIRONMENT-GAMEPLAY | One readable environment slice—surface, concealment, hazard, traversal device, interactive geometry, or ability-created region—using server-owned outcomes | Concealment/reveal completed in V9. Promoted into active V10: M01's oil-barrel slice is at specification review and M03 owns the later treasure-chest/restoration-pickup slice. Unrelated hazards, traversal devices, interactions, and temporary regions remain candidates | [V10 roadmap](./implementation/v10/roadmap.md); [damageable objects and Heist specification](./18-damageable-world-objects-and-heist.md); [V9 roadmap](./implementation/v9/roadmap.md); [concealment and reveal specification](./17-concealment.md); [environment gameplay direction](./09-environment-gameplay.md). V10 M01 owns the retained `M08-ENV-SOURCE` obligation in the [V1 backlog](./implementation/v1/roadmap.md#v1-backlog) |
| CAND-MAP-CONTENT | Additional built-in maps or themes using the implemented sparse-grid map-asset contract | V8 converted every existing built-in and delivered Tidal Garden as the reference-grammar proof; further production maps/themes remain a later content decision | [V8 roadmap](./implementation/v8/roadmap.md); [grid map-asset specification](./16-grid-map-asset-system.md); [V4 historical roadmap](./implementation/v4/roadmap.md) |
