...
Last reconciled: **2026-08-23**, after V8's map-system cutover entered user playtest and
concealment/reveal was promoted into the V9 specification-review roadmap.
...
| CAND-ENVIRONMENT-GAMEPLAY | One readable environment slice—surface, concealment, hazard, traversal device, interactive geometry, or ability-created region—using server-owned outcomes | Concealment/reveal is promoted to V9 with terrain, self-cloak, allied-area, proximity, attack/damage reveal, and reveal-scan slices. V8 already delivered water, non-concealing vegetation, destruction, and replacement; unrelated hazards, traversal devices, and other temporary regions remain candidates | [V9 roadmap](./implementation/v9/roadmap.md); [concealment and reveal specification](./17-concealment.md); [V8 roadmap](./implementation/v8/roadmap.md); [environment gameplay direction](./09-environment-gameplay.md). The first environmental damage author must also revisit `M08-ENV-SOURCE` in the [V1 backlog](./implementation/v1/roadmap.md#v1-backlog) |
