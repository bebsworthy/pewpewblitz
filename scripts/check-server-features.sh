...
for forbidden in bevy_render bevy_winit bevy_window bevy_audio bevy_asset bevy_sprite bevy_text bevy_ui bevy_ui_render bevy_gilrs rodio lewton symphonia; do
...
if grep -qE 'feature "(png|vorbis|aac|flac|mp3|wav)"' <<< "$features"; then
  echo "server feature graph unexpectedly contains a client media decoder" >&2
  exit 1
fi
